...
class RobloxTracker:
    ...
    async def execute_tracking(self):
        ...
        # 4. CONTINUE / IDLE: State is the same as the cache.
        else:
            if current_tracking:
...
def run_dummy_webserver(host=DUMMY_WEB_SERVER_HOST, port=DUMMY_WEB_SERVER_PORT):
    ...
    # Log the exact port being used
    print(f"--- RENDER HEALTH CHECK STARTING: Attempting to open port {port} on {host} ---")
    
    try:
        httpd = HTTPServer(server_address, HealthCheckHandler)
        logging.info(f"SUCCESS: Dummy web server is now serving on http://{host}:{port}")
        httpd.serve_forever()
    except Exception as e:
        logging.error(f"FATAL ERROR: Failed to start dummy web server on port {port}. Check if the port is busy. Error: {e}")
...
if __name__ == '__main__':
    try:
        ...
        # Catch any unexpected errors during startup or loop execution
